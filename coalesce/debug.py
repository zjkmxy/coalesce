...
from radix import radix_8, radix_16, radix_32, radix_128, radix_256, Hex

log_print = False

def f_k_int_x(k: str, x: int):
    aes_obj = AES.new(bytes.fromhex(k), AES.MODE_ECB)
    s = ""
    for i in range(1,4):
        ...
        if log_print:
            print("0x" + xpi)
            cArrayDef("[be]", "xp" + str(i), int(xpi, 16), 128//8, radix_8, False); print(os.linesep)

        if log_print: print("AES_k(x+" + str(i) + "): Output of AES block " + str(i) + " encryption (128 bits):")
        aes_xpi = aes_obj.encrypt(bytes.fromhex(xpi)).hex()
        if log_print:
            print("0x" + aes_xpi.upper())
            cArrayDef("[be]", "aes_xp" + str(i), int(aes_xpi, 16), 128//8, radix_8, False); print(os.linesep)
...
def bfexpandkey(i, j, exp, seedprv, type="cert"):
    """ Butterfly expansion for "cert" and "enc" keys
        ...
        i, j:    {int} are the i, j values for the corresponding certificate
        exp:     {int or octet string} expansion value
        seedprv: {int or octet string} seed private key
    ...
    # ensure seedprv is a int
    if isinstance(seedprv, str):
        seedprv = int(seedprv, 16)

    f_k_x = int(f_k_int_x(exp, x), 16) % genP256.ecc.n
...
""")
    ...
    print("0x" + ck)
    cArrayDef("[be]", "ck", int(ck, 16), 128//8, radix_8, False); print(os.linesep)
    ...
    cArrayDef("[be]", "f_k_int_x_cert", int(f_k_int_x_cert, 16), 384//8, radix_8, False); print(os.linesep)
    ...
    print("0x" + ek)
    cArrayDef("[be]", "ek", int(ek, 16), 128//8, radix_8, False); print(os.linesep)
    ...
    cArrayDef("[be]", "f_k_int_x_enc", int(f_k_int_x_enc, 16), 384//8, radix_8, False); print(os.linesep)
